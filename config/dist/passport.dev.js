...
var LocalStrategy = require('passport-local').Strategy; // Generate Hash
...
var RunQuery = database.RunQuery; // expose this function to our app using module.exports
...
module.exports = function (passport) {
  ...
  passport.use('sign-in', new LocalStrategy({
    ...
    RunQuery(sqlStr, function (rows) {
      ...
      if (rows.length < 1) return done(null, false, req.flash('signInError', 'No user found.')); // req.flash is the way to set flashdata using connect-flash
      // if the user is found but the password is wrong

      if (!bcrypt.compareSync(password, rows[0].Password)) return done(null, false, req.flash('signInError', 'Oops! Wrong password.')); // create the loginMessage and save it to session as flashdata
            ...
            if (usernameRows.length > 0) {
              ...
              RunQuery(insertQuery, function (insertResult) {
                ...
                insertQuery = 'INSERT INTO Addresses\
                                    VALUES(null, ' + insertResult.insertId + ', \'' + fullName + '\', \'' + address + '\', \'' + postcode + '\', \'' + city + '\', \'' + country + '\', \'' + phone + '\')';
