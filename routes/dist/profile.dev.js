...
var bcrypt = require('bcrypt-nodejs'); // database module
...
function isLoggedIn(req, res, next) {
  ...
  if (req.isAuthenticated()) return next(); // if they aren't redirect them to the home page
...
router.route('/:username/change-password').get(isLoggedIn, function (req, res) {
  ...
  } else {//passwords are not matched
    }
});
router.route('/:username/orders').get(isLoggedIn, function (req, res) {
  var selectQuery = '\
            SELECT *\
            FROM Orders\
            WHERE UserID = ' + req.user.UserID;
...
router.route('/:username/orders/ll').get(isLoggedIn, function (req, res) {
  res.render('profile/pay');
});
router.route('/eye-test/eye').get(isLoggedIn, function (req, res) {
  res.render('views/eye/');
});
router.route('/:username/orders/:id').get(isLoggedIn, function (req, res) {
  //get order info
  var selectQuery = '\
            SELECT *\
            FROM Orders\
...
router.route('/:username/addresses').get(isLoggedIn, function (req, res) {
  var selectQuery = '\
            SELECT *\
            FROM Addresses\
            WHERE UserID = ' + req.user.UserID;
...
router.route('/:username/addresses/:id/edit').get(isLoggedIn, function (req, res) {
  var selectQuery = '\
            SELECT *\
            FROM Addresses\
...
router.route('/:username/addresses/:id/delete').post(isLoggedIn, function (req, res, next) {
  var sqlStr = '\
...
router.route('/:username/addresses/add').get(isLoggedIn, function (req, res) {
...
router.route('/:username/addresses/:id/ll').get(isLoggedIn, function (req, res) {});
