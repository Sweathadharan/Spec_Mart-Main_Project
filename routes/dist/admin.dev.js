...
var slug = require('slug'); // database module
...
router.route('/cat/:id/edit').get(isAdmin, function (req, res, next) {
  var sqlStr = '\
        SELECT *\
        FROM Categories\
...
router.route('/cat/add').get(isAdmin, function (req, res, next) {
  var contextDict = {
...
router.route('/products/:id/edit').get(isAdmin, function (req, res, next) {
  var sqlStr = '\
                    SELECT Products.*, Categories.CategoryName\
                    FROM Products\
                    INNER JOIN Categories\
...
router.route('/products/add').get(isAdmin, function (req, res, next) {
  var sqlStr = '\
            SELECT *\
            FROM Categories';
  RunQuery(sqlStr, function (categories) {
  ...
  var sqlStr = "\
            ...
            VALUES (null, '" + req.body.name + "', " + req.body.category + ', ' + req.body.price + ', ' + req.body.unit + ", \
            '" + req.body.description + "', " + req.body.year + ", \
            '" + slug(req.body.name) + ".png', \
            '" + slug(req.body.name) + "', " + req.body.feature + ')';
  /*Image = name.png\*/
...
router.route('/orders/:id/update').get(isAdmin, function (req, res, next) {
  var selectQuery = '\
            SELECT *\
            FROM Orders\
            WHERE OrderID = ' + req.params.id;
